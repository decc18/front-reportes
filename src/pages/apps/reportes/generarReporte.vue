...
const getChipColor =  (estado: number) => {
      switch (estado) {
        ...
        default:
          return 'info'; // color por defecto (azul)
      }
    }
...
const mostrarPopUpCrearReporte = async (id: number, descrip: string) => {
  ...
  const datosReporte = await $api(`api/Reporte/ConsultarCamposReporte?idReporte=${id}`, { method: 'GET' })
  const camposReporte = computed((): typCampo[] => datosReporte)
...
const handleSubmit = async () => {
  ...
  if (validationResult?.valid) {
    console.log('Formulario válido, enviar datos:', formValues.value);
    const formValuesString = JSON.stringify(formValues.value);
    console.log('Formulario válido, enviar datos:', formValuesString);
    ...
    console.log('Formulario no válido', validationResult?.errors);
    // Mostrar los errores o realizar otras acciones
  }
};
              ...
              >
                ...
                <template v-else-if="campo.tipoComponente === 'Select' && campo.visible === true">
                  ...
                  :disabled="!campo.itemsLista?.length"
                    ...
                    item-title="valor" item-value="id"
                    :required="campo.requerido"
                    :rules="[campo.requerido ? (v: string | number) => !!v || campo.mensajeError : null]"
                    @update:modelValue="changeValueItem($event, campo.catalogo)"
